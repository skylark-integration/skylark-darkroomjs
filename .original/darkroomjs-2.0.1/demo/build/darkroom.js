...
function() {
    ...
    Darkroom.prototype = {
        ...
        constructor: function(element, options, plugins) {
            if (this.options = Darkroom.Utils.extend(options, this.defaults),
        ...
        _popTransformation: function(transformations) {
            if (0 === transformations.length)
                return this.dispatchEvent("core:reinitialized"),
                void this.refresh();
...
function() {
    ...
    Plugin.extend = function(protoProps) {
        ...
        protoProps && Darkroom.Utils.extend(child.prototype, protoProps),
...
function() {
    ...
    Toolbar.prototype = {
        createButtonGroup: function(options) {
            ...
            return buttonGroup.className = "darkroom-button-group",
    ...
    ButtonGroup.prototype = {
        createButton: function(options) {
            ...
            return button.hide(options.hide),
...
function() {
    ...
    function extend(b, a) {
        ...
        if (void 0 === b)
            return a;
        for (prop in a)
            a.hasOwnProperty(prop) && b.hasOwnProperty(prop) === !1 && (b[prop] = a[prop]);
        return b
    }
...
function(window, document, Darkroom, fabric) {
    ...
    Darkroom.plugins.history = Darkroom.Plugin.extend({
        ...
        _updateButtons: function() {
            this.backButton.disable(0 === this.darkroom.transformations.length),
            this.forwardButton.disable(0 === this.undoTransformations.length)
        },
    ...
    Darkroom.plugins.crop = Darkroom.Plugin.extend({
        ...
        onObjectScaling: function(event) {
            if (this.hasFocus()) {
                ...
                if (currentObject === this.cropZone) {
                    ...
                    if (null !== this.options.ratio && (0 > minX || maxX > canvas.getWidth() || 0 > minY || maxY > canvas.getHeight()) && (preventScaling = !0),
                    0 > minX || maxX > canvas.getWidth() || preventScaling) {
                        var lastScaleX = this.lastScaleX || 1;
                        currentObject.setScaleX(lastScaleX)
                    }
                    if (0 > minX && currentObject.setLeft(0),
                    0 > minY || maxY > canvas.getHeight() || preventScaling) {
        ...
        onMouseDown: function(event) {
            if (this.hasFocus()) {
                ...
                activeObject === this.cropZone || this.cropZone.containsPoint(point) || (canvas.discardActiveObject(),
        ...
        onMouseMoveKeyCrop: function(event) {
            var canvas = this.darkroom.canvas
              ...
              , x = pointer.x
              , y = pointer.y;
            zone.left && zone.top || (zone.setTop(y),
            zone.setLeft(x)),
        ...
        onKeyDown: function(event) {
            !1 === this.options.quickCropKey || event.keyCode !== this.options.quickCropKey || this.isKeyCroping || (this.isKeyCroping = !0,
        ...
        onKeyUp: function(event) {
            !1 !== this.options.quickCropKey && event.keyCode === this.options.quickCropKey && this.isKeyCroping && (this.isKeyCroping = !1,
            this.startX = 1,
            this.startY = 1,
            this.onMouseUp())
        },
        ...
        releaseFocus: function() {
            void 0 !== this.cropZone && (this.cropZone.remove(),
        ...
        _renderCropZone: function(fromX, fromY, toX, toY) {
            ...
            0 > leftX && (rightX += Math.abs(leftX),
            leftX = 0),
            rightX > canvas.getWidth() && (leftX -= rightX - canvas.getWidth(),
            rightX = canvas.getWidth()),
            0 > topY && (bottomY += Math.abs(topY),
            topY = 0),
            bottomY > canvas.getHeight() && (topY -= bottomY - canvas.getHeight(),
            ...
            if (this.options.ratio && +this.options.ratio !== currentRatio) {
                ...
                if (this.isKeyCroping && (isLeft = this.isKeyLeft,
                isUp = this.isKeyUp),
                ratio > currentRatio) {
